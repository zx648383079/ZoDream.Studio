...
namespace ZoDream.Shared.Utils
{
    ...
    {
        public static int IndexOf<T>(IList<T> items, Func<T, bool> compareFn)
        {
        ...
        public static void Remove<T>(IList<T> items, Func<T, bool> compareFn)
        {
            Remove(items, compareFn, -1);
        }

        public static void Remove<T>(IList<T> items, Func<T, bool> compareFn, int excludeIndex)
        {
            for (int i = items.Count - 1; i >= 0; i--)
            {
                if (i == excludeIndex)
